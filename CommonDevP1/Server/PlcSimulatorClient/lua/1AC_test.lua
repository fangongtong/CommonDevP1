...
function DealReal()
    ...
    while true do
		...
		print("DealReal 1")
		Common.UnpackRealDt({coroutine.yield()},realDt)
		--print("DealReal 2")
		--print(realDt.Com2)
		if realDt.Break == true then 
			break
		end
		
		--  如果还没有更新次数,那就更新本次最大力值
		cyliderRecordDt.syncCrntForce = realDt.CR[1].RealForce
		if cyliderRecordDt.syncCount - cyliderRecordDt.lastCount < 2 then
			if cyliderRecordDt.syncMaxForce < realDt.CR[1].RealForce then
				cyliderRecordDt.syncMaxForce = realDt.CR[1].RealForce
			end
		else
